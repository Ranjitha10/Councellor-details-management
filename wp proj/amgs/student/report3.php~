...
$pdf->AddPage();
		...
		$pdf->Image('Library/fpdf/logo.png',40,8.00125,0,18);
				...
				$dispsem=6;
				$pdf->Multicell(0, 5 , 'I am herewith furnishing the progress report of your ward Mr./Ms. '.$name.' bearing Roll No: '.$usn.' of '.$dispsem.' Semester for the Third internal assessmenet examination as under. You are here by requested to go through the report and give us your feeback.');
				$pdf->Ln(4);
				...
				for($i=0;$i<$subct;$i++)
				{
					...
					if($whichinternal=="internal1")
						$attendance=mysqli_query($con,"select * from att1 where USN='$usn'")
									or die(mysqli_error());
					else if($whichinternal=="internal2") $attendance=mysqli_query($con,"select * from att2 where USN='$usn'")
									or die(mysqli_error());
					else $attendance=mysqli_query($con,"select * from att3 where USN='$usn'")
									or die(mysqli_error());
